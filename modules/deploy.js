...
const deploy = () => {
    ...
    const platforms = (data = [], op = {}) => {
        ...
        const deploySaomai = (dataSource, op) => {
            ...
            // 提示信息callback
            let getTipStr = (set, loopSet) => `${_.capitalize(loopSet.platform)} ${op.tipType.toLowerCase()}'s ${loopSet.data[loopSet.count].name}`;
                    ...
                    return new Promise((resolve, reject) => {
                        ...
                        if (fs.existsSync(loopSets[count].data[0].folder)) {
                            if (set.rebuild === true) {
                                ...
                                log.blue(`${set.tipType} ${_.capitalize(set.tipStr ? set.tipStr : set.platform)} start checkout...`, true);
                                log.time("rebuild folder：disable");
